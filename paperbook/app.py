...
def signup(username: str = Form(...), email: str = Form(""),
    ...
    user = db.create_user(username, email.strip(), pwctx.hash(password))
...
def login(username: str = Form(...), password: str = Form(...)):
    ...
    if not user or not pwctx.verify(password, user["pw_hash"]):
...
def mybets(request: Request):
    ...
    return templates.TemplateResponse("mybets.html", {
        "request": request, "user": user, "bets": db.user_bets(user["id"])})
